local groups
net.Receive("TabMenuRefresh", function()
    groups = net.ReadString()
end)

function TabMenuAdmins(plygroup)
    if not groups then return false end
    local tabgroup = string.Explode(";", groups)
    local toreturn = false
    for _, grp in pairs(tabgroup) do
        if grp == plygroup then toreturn = true break end
    end
    return toreturn
end
...
function PANEL:Init()
    ...
    if TabMenuAdmins(LocalPlayer():GetUserGroup()) then
...
function PANEL:Paint()
    ...
    local color = Color(20, 150, 20, 255)
